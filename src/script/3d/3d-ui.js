...
function buildExtrusionControlsFromState() {
  ...
  colors.forEach(color => {
    ...
    controlDiv.innerHTML = `<div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;"><div style="width:14px;height:14px;border-radius:3px;background:${color};border:1px solid var(--border);flex-shrink:0;"></div><span style="flex:1;font-size:10px;font-weight:600;color:var(--text);">${getColorName(color)}</span><button style="width:24px;height:24px;display:flex;align-items:center;justify-content:center;background:${isVisible?'transparent':'#ff4444'};border:1px solid var(--border);border-radius:4px;cursor:pointer;color:${isVisible?'var(--text)':'white'};font-size:14px;padding:0;flex-shrink:0;" onclick="toggleColorVisibility('${color}')">👁</button></div><div style="display:flex;align-items:center;gap:6px;"><input type="range" min="0" max="100" value="${threeExtrusionLevels[color].extrusion}" style="flex:1;accent-color:var(--accent);"><input type="number" min="0" max="200" value="${threeExtrusionLevels[color].extrusion}" class="prop-input" style="flex-shrink:0;"><span style="color:var(--muted);font-size:10px;min-width:16px;">px</span></div>`;
